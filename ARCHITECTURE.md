...
Das eingebaute Asgaros-Abonnement-Control wird über den Adapter einmalig als
kontextabhängiger Nav-Eintrag direkt vor „Abonnements“ eingeordnet. URLs,
Nonces und der Abonnementzustand werden nicht dupliziert.
...
- Die globale Hub-Navigation enthält links eine dauerhafte Schaltfläche `Forum` (Link zur Forum-Startseite) sowie hubweite Ansichten (`Meine Arbeitsgruppen`, `Meine Einladungen`, `Arbeitsgruppen entdecken`, optional `Arbeitsgruppe gründen`).
  - **MVP 3.1 und 3.2:** Die fachliche Begrifflichkeit wird ab MVP 3.1 auf "Arbeitsgruppe" und "Arbeitsgruppenverantwortliche" umgestellt (statt "Raum" und "Raumverantwortliche").
  - **MVP 4:** Raumgründung wird danach implementiert, basierend auf der etablierten Arbeitsgruppenlogik.
- Raumbezogene Verwaltungstabs (`Mitglieder`, `Einladungen`, `Beitrittsanfragen`) werden nur im Verwaltungskontext und direkt unter einem Arbeitsgruppenüberschrift angezeigt.
- `ForumNavigation` hängt über dokumentierte Asgaros-Hooks (`asgarosforum_filter_header_menu`, `asgarosforum_overview_custom_content_top`) einen Menüpunkt "Arbeitsgruppen" und ein Einstiegs-Panel in das Forum ein (seit MVP 3.2).
...
- Das Erscheinungsbild wird standardmäßig Asgaros-nah ausgeliefert und kann über die Admin-Seite `Einstellungen -> AFSpaces Look & Feel` konfiguriert werden. Die Werte werden als Option gespeichert und per Inline-CSS auf den Frontend-Style angewendet. Zusätzlich stehen Presets (`Asgaros-Nah`, `Neutral`, `Kontrastreich`) sowie ein Reset auf Standardwerte zur Verfügung.
- Erweiterungspunkte (`afspaces_hub_navigation_tabs`, `afspaces_render_space_creation`, Filter `afspaces_enable_space_creation`) ergänzen die Arbeitsgruppengründung. Die Sichtbarkeit und serverseitige Zulässigkeit werden zentral über `SpaceCreationService::can_user_create()` und `SpaceCreationSettings::OPTION` entschieden.
